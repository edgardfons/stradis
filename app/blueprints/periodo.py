...
from app.models.periodo import Professor, ProfessorCreateForm, ProfessorIndexForm, Status
...
def index(page):
    ...
    periodo = ProfessorIndexForm()
    periodos = Professor.query
    ...
    periodos = periodos.filter( Professor.descricao.ilike(sql_ilike_format(periodo.descricao.data)) ) if periodo.descricao.data else periodos
    periodos = periodos.filter( Professor.vencimento >= sql_date_format(periodo.inicio.data) ) if periodo.inicio.data else periodos
    periodos = periodos.filter( Professor.vencimento <= sql_date_format(periodo.fim.data) ) if periodo.fim.data else periodos

    periodos = periodos.order_by(Professor.codigo.desc()).paginate(page, per_page=per_page, error_out=True)
...
def new():
    ...
    if titulo_form.validate_on_submit():
        ...
        if titulo_form.codigo.data:
            periodo = Professor.query.get_or_404( titulo_form.codigo.data )
...
def view(titulo_id):
    periodo = Professor.query.get_or_404(titulo_id)
    titulo_form = ProfessorCreateForm()
